...
outer:
	...
	*/
	...
	// 		ISAAC Custom:
	//
	// 		Currently: The tails of stops (also, the beginning of trips) as I see them on the map
	// 		 are mislabeled as Tripping=false. Around 10 points, keep in mind those vector
	// 		points have been generated by tippecanoe, so they are not the raw data points,
	// 	    and can be clustered.
	//
	// 		So I want to increase the sensitivity for the beginnings of trips.
	// 		I think acceleration might do it.
	// 		If the acceleration is rising for N points, then we are starting a trip.
	// */
	//
	// acceleration := []float64{}
	// for i := 1; i < len(d.intervalPoints); i++ {
	// 	prev := d.intervalPoints[i-1]
	// 	if prev.MustGetTime().Before(t.MustGetTime().Add(-d.DwellTime)) {
	// 		continue
	// 	}
	// 	cur := d.intervalPoints[i]
	// 	// Get the time difference between the two points.
	// 	dur := cur.MustGetTime().Sub(prev.MustGetTime())
	// 	// Get the distance between the two points.
	// 	dist := geo.Distance(prev.Point(), cur.Point())
	// 	// Get the speed.
	// 	speed := dist / dur.Seconds()
	// 	// Get the acceleration.
	// 	acceleration = append(acceleration, speed/dur.Seconds())
	// }
	// accelerationStats := stats.Float64Data(acceleration)
	//
